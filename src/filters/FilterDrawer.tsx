...
export function FilterDrawer({ isOpen, onClose, pageType, categoryId }: { isOpen: boolean; onClose: () => void; pageType: "search" | "products" | "category" | "deals"; categoryId?: string }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Filtros avançados" onClick={onClose}>
    <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-border bg-background shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <footer className="flex gap-3 border-t border-border p-5"><button type="button" onClick={() => { setDraft(reset); api.setFilters(reset); api.updateURL(reset); }} className="flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm"><RotateCcw className="h-4 w-4"/>Limpar</button><button type="button" disabled={api.loading} onClick={() => { api.setFilters(draft); api.updateURL(draft); onClose(); }} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50"><Filter className="h-4 w-4"/>Aplicar {api.loading ? "…" : `(${api.totalResults.toLocaleString("pt-BR")})`}</button></footer>
    </aside>
  </div>;
}
