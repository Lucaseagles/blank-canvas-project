...
export function FilterDrawer({ isOpen, onClose, pageType, categoryId }: { isOpen: boolean; onClose: () => void; pageType: "search" | "products" | "category" | "deals"; categoryId?: string }) {
  ...
  useEffect(() => { if (isOpen) { setDraft(api.filters); setClosing(false); } }, [api.filters, isOpen]);
  useEffect(() => {
    if (!isOpen) return;
  ...
  const remove = (key: keyof FilterState, value?: string) => {
    if (key === "categories" || key === "marketplaces") setDraft({ ...draft, [key]: draft[key].filter((x) => x !== value) });
    else setDraft({ ...draft, [key]: key === "minPrice" ? api.availableFilters.minPrice : key === "maxPrice" ? api.availableFilters.maxPrice : key === "minDiscount" || key === "minRating" ? 0 : false });
  };
  ...
  return (
    <div className={`fixed inset-0 z-[100] bg-black/60 backdrop-blur-md transition-opacity duration-300 ${closing ? "opacity-0" : "opacity-100"}`} role="dialog" aria-modal="true" aria-label="Filtros avançados" onClick={onClose}>
      <aside className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-border/60 bg-background/95 shadow-2xl backdrop-blur-2xl transition-transform duration-300 motion-safe:animate-[filter-drawer-in_300ms_cubic-bezier(.16,1,.3,1)] ${closing ? "translate-x-full" : "translate-x-0"}`} onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-border/60 bg-card/40 p-5">
          <div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><SlidersHorizontal className="h-5 w-5" /></div><h2 className="font-black tracking-tight">Filtros avançados</h2><span className="min-w-6 rounded-full bg-primary/10 px-2 py-1 text-center text-xs font-bold text-primary">{active}</span></div>
          <button aria-label="Fechar filtros" onClick={onClose} className="rounded-xl p-2 text-muted-foreground transition hover:bg-muted hover:text-foreground"><X className="h-5 w-5" /></button>
        </header>
        ...
        <footer className="flex items-center gap-3 border-t border-border/60 bg-card/50 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          <button type="button" onClick={clear} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-border px-4 text-sm font-bold text-muted-foreground transition hover:bg-muted hover:text-foreground"><RotateCcw className="h-4 w-4"/> Limpar</button>
          <button type="button" disabled={api.loading} onClick={apply} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/15 transition hover:brightness-105 disabled:cursor-wait disabled:opacity-60"><Filter className="h-4 w-4"/> Aplicar {api.loading ? "…" : `(${api.totalResults.toLocaleString("pt-BR")})`}</button>
        </footer>
      </aside>
    </div>
  );
