...
/**
 * Resolves and persists the A/B assignment using the verified server-side
 * Supabase client. The browser never receives direct write access to the
 * assignment flow.
 */
export const getAbVariantServer = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: experiment, error: experimentError } = await db
      .from("ab_experiments")
      .select("variant_a,variant_b,is_active")
    ...
    // Stable deterministic assignment prevents reassignment while retaining
    // the database uniqueness guarantee on (experiment_id, user_id).
    const variant =
      Array.from(data.experimentId + context.userId).reduce(
        (n: number, c: string) => (n * 31 + c.charCodeAt(0)) >>> 0,
        7,
      ) % 2 === 0
        ? "A"
        : "B";
    ...
    // Another request may have won the unique constraint race. Read the
    // persisted assignment rather than returning a potentially different one.
...
/** Owner-only access to the existing protected database result function. */
