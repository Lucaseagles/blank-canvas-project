...
const SECRET_KEYS = new Set([
  "token", "access_token", "refresh_token", "api_key", "apikey", "secret", "password", "client_secret", "authorization", "bot_token", "service_role_key",
]);
...
export const getAdminSettings = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    const results = await Promise.all(SETTINGS_REGISTRY.map(async (entry) => {
      const { data, error } = await db.from(entry.table).select("*").order("updated_at", { ascending: false, nullsFirst: false }).limit(100);
      if (error) return { ...entry, rows: [], available: false, error: error.message };
      return { ...entry, rows: (data ?? []).map((row: Record<string, unknown>) => sanitize(row)), available: true, error: null };
    }));
    return results;
  });
...
export const updateAdminSetting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if ("updated_at" in patch === false) patch.updated_at = new Date().toISOString();
    ...
    await db.rpc("append_admin_audit", {
...
export const getAdminAuditLog = createServerFn({ method: "GET" })
  .middleware([requireOwnerRole])
  .validator((data: unknown) => z.object({ page: z.number().int().min(0).default(0), pageSize: z.number().int().min(1).max(100).default(25), actionType: z.string().optional(), entityType: z.string().optional() }).parse(data))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const db = supabaseAdmin as any;
    ...
    if (error) throw error;
    return { rows: (rows ?? []).map((row: Record<string, unknown>) => sanitize(row)), count: count ?? 0, page: data.page, pageSize: data.pageSize };
  });
