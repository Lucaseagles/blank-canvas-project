...
/**
 * Single registry for the existing configuration sources. This is deliberately
 * schema-driven: the admin hub edits existing tables only and never creates a
...
const SECRET_KEYS = new Set([
  ...
  "password", "client_secret", "authorization", "bot_token", "service_role_key",
  "bot_token_secret_ref", "access_token_secret_ref", "api_config",
]);

const HAS_UPDATED_AT = new Set([
  "personalization_weights", "feed_mix_config", "social_proof_config", "automation_rules",
  "app_points_config", "feature_flags", "whatsapp_config", "telegram_config",
]);
...
function sanitize(value: unknown): SerializableValue {
  ...
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>).map(([key, nested]) => [
      key,
      SECRET_KEYS.has(key.toLowerCase()) ? "[REDACTED]" : sanitize(nested),
    ]),
  );
}
...
export const getAdminSettings = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    return Promise.all(
      SETTINGS_REGISTRY.map(async (entry) => {
        const { data, error } = await db.from(entry.table).select("*").limit(100);
        if (error) {
          return { ...entry, rows: [] as SerializableValue[], available: false, error: String(error.message) };
        }
        const rows = [...(data ?? [])]
...
export const updateAdminSetting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // The audit call uses the verified owner-bound client from the middleware.
    // This preserves auth.uid() instead of calling the audit RPC through the
    // service-role client, where auth.uid() would be null.
...
export const getAdminAuditLog = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    let query = db
      ...
      .select("id,actor_user_id,actor_email,action_type,entity_type,entity_id,previous_value,new_value,created_at", { count: "exact" })
