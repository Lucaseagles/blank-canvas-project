...
export async function createBundle(data: any) {
  ...
  if (products) {
    const bundleProducts = products.map((p: any) => ({ bundle_id: bundle.id, product_id: p.id, position: p.position }));
...
export async function updateBundle(id: string, data: any) {
  ...
  if (products) {
    ...
    if (products.length > 0) {
      const bundleProducts = products.map((p: any) => ({ bundle_id: id, product_id: p.id, position: p.position }));
      const { error: productsError } = await supabaseAdmin.from('bundle_products').insert(bundleProducts);
