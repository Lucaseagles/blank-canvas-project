...
import { deriveAdminIntegrationState, type IntegrationDbStatus } from "@/integrations/core/database";
...
export const saveIntegrationSecret = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await db.rpc("set_integration_secret", { p_platform_id: data.platformId, p_field_key: data.fieldKey, p_value: data.value });
...
export const getIntegrationStatuses = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const db = supabaseAdmin as any;
    const { data, error } = await db.from("integration_credentials").select("platform_id, status, last_checked_at, last_error, created_at, updated_at").order("platform_id");
    if (error) throw new Error(`Não foi possível carregar as integrações: ${error.message}`);
    return data ?? [];
  });

export const getIntegrationFeatureState = createServerFn({ method: "GET" })
  .middleware([requireOwnerRole])
  .handler(async () => {
...
export const testIntegration = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const db = supabaseAdmin as any;
    const { error } = await db.rpc("mark_integration_check", { p_platform_id: data.platformId, p_status: result.success ? "active" : "error", p_error: result.success ? null : (result.message ?? result.error ?? "Falha na conexão") });
    if (error) console.error("Failed to persist integration test status", error);
    return { success: result.success, ...(result.message ? { message: result.message } : {}), ...(result.error ? { error: result.error } : {}) };
  });
