...
export const getActiveSocialChannels = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("social_channels").select("id,platform,channel_name,channel_url,icon,is_active,created_at").eq("is_active", true).order("platform").order("channel_name");
  if (error) throw error;
  return data ?? [];
});

export const getAdminSocialChannels = createServerFn({ method: "GET" }).middleware([requireOwnerRole]).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("social_channels").select("*").order("platform").order("channel_name");
  if (error) throw error;
  return data ?? [];
});
...
export const deleteSocialChannel = createServerFn({ method: "POST" }).middleware([requireOwnerRole]).validator((data: unknown) => z.object({ id: z.string().uuid() }).parse(data)).handler(async ({ data }) => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { error } = await supabaseAdmin.from("social_channels").delete().eq("id", data.id);
  if (error) throw error;
  return { success: true };
});

export const getCrossPromoAnalytics = createServerFn({ method: "GET" }).middleware([requireOwnerRole]).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.rpc("get_social_cross_promo_summary");
  if (error) throw error;
  return data ?? [];
});
