...
export function ShareCardCanvas({ referralCode, referralLink, userName, reward, format, config, onGenerated }: Props) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const qrSize = format === "9_16" ? 180 : 220, qrCanvas = document.createElement("canvas"); await QRCode.toCanvas(qrCanvas, referralLink, { width: qrSize, margin: 1, color: { dark: "#FFFFFF", light: "#00000000" } }); if (cancelled) return; roundedRect(ctx, (width - qrSize - 28) / 2, (format === "9_16" ? 382 : 485), qrSize + 28, qrSize + 28, 16); ctx.fillStyle = "rgba(255,255,255,.08)"; ctx.fill(); ctx.drawImage(qrCanvas, (width - qrSize) / 2, format === "9_16" ? 396 : 499);
