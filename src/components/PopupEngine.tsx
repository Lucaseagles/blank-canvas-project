...
import { X, ArrowRight, Clock3, Sparkles, Target, Package, Zap, Crown } from "lucide-react";
...
export function PopupEngine() {
  ...
  useEffect(()=>{ if(remaining===null)return; const id=window.setInterval(()=>setRemaining(Math.max(0,remaining-(Date.now()-Date.now()))),1000); return()=>window.clearInterval(id); },[]);
  ...
  const record=async(eventType:"click"|"dismiss",target?:string)=>{const {data:{user}}=await supabase.auth.getUser(); await (supabase as any).rpc("record_popup_event",{p_rule_id:popup!.rule_id,p_user_id:user?.id??null,p_event_type:eventType}); setPopup(null); shown.current=false; if(target)window.location.assign(target)};
  ...
  const icon=isBundle?<Package className="h-4 w-4"/>:isFlash?<Zap className="h-4 w-4"/>:isUpsell?<Crown className="h-4 w-4"/>:<Sparkles className="h-4 w-4"/>;
  const title=isBundle?'🎁 Kit Exclusivo':isFlash?'⚡ Oferta Relâmpago':isUpsell?'⬆ Upsell Inteligente':content.title??popup.name;
