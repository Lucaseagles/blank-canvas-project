      ...
      {isOpen && (
        <section id="support-floating-chat" aria-label="Central de suporte" className="fixed bottom-[148px] right-3 z-[109] flex h-[min(620px,72vh)] w-[min(390px,calc(100vw-24px))] flex-col overflow-hidden rounded-[28px] border border-white/10 bg-background/95 shadow-2xl shadow-black/30 backdrop-blur-2xl md:bottom-[88px] md:right-6">
          ...
          <div className="shrink-0 space-y-2 border-t border-white/10 bg-background/90 p-3">
            <button type="button" onClick={() => void handleWhatsApp()} disabled={!whatsappNumber} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[#25D366] px-4 py-3 text-sm font-black text-white transition-transform hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-50"><Phone className="h-4 w-4" />{whatsappNumber ? "Falar com um humano no WhatsApp" : "WhatsApp não configurado"}</button>
