...
import { Clock3, Trash2, ArrowRight, PlayCircle } from 'lucide-react';
...
function metadataVideoId(metadata: unknown): string | undefined {
  if (!metadata || Array.isArray(metadata) || typeof metadata !== 'object') return undefined;
  const value = (metadata as Record<string, unknown>)['video_id'];
...
async function getRecent(limit = 8): Promise<RecentItem[]> {
  ...
  const { data: events, error } = await supabase.from('analytics_events').select('id,event_type,product_id,created_at,metadata').eq('user_id', auth.user.id).in('event_type', ['PRODUCT_VIEW', 'VIDEO_START', 'VIDEO_PROGRESS']).order('created_at', { ascending: false }).limit(Math.max(limit * 8, 80));
  if (error) throw error;
  const seen = new Set<string>(); const result: RecentItem[] = [];
  for (const event of events || []) {
    const videoId = metadataVideoId(event.metadata);
    ...
    seen.add(key); result.push({ key, viewedAt: event.created_at, ...(videoId ? { videoId } : {}) });
  ...
  const videoIds = result.flatMap(item => item.videoId ? [item.videoId] : []);
  if (videoIds.length) {
