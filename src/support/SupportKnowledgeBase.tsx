...
type SupportKnowledgeBaseProps = { initialQuestion?: string };

export function SupportKnowledgeBase({ initialQuestion = "" }: SupportKnowledgeBaseProps) {
  const [items, setItems] = useState<Faq[]>([]);
  const [question, setQuestion] = useState(() => initialQuestion.trim().slice(0, 300));
  ...
  const save = async () => {
    const cleanQuestion = question.trim(); const cleanAnswer = answer.trim();
