...
import { ArrowLeft, Award, BellRing, Heart, Layers, ShieldCheck, Star, Target, Zap } from 'lucide-react';
...
function ProductDetailPage() {
  ...
  const [targetPrice, setTargetPrice] = useState(''); const [alertOpen, setAlertOpen] = useState(false); const [showDownsell, setShowDownsell] = useState(false); const [videoResumeAt, setVideoResumeAt] = useState(0);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      trackEvent('PRODUCT_VIEW', { product_id: product.id, category_id: product.category_id || null }, { productId: product.id, categoryId: product.category_id || undefined });
      if (!user) return;
      const { data: favorite } = await supabase.from('favorites').select('id').eq('user_id', user.id).eq('product_id', product.id).maybeSingle();
      if (favorite) setIsFavorited(true);
      const videoId = product.video_products?.[0]?.video_id as string | undefined;
      if (videoId) {
        const { data: progress } = await (supabase as any).from('video_watch_progress').select('last_watched_seconds').eq('user_id', user.id).eq('video_id', videoId).maybeSingle();
        if (progress?.last_watched_seconds) setVideoResumeAt(Number(progress.last_watched_seconds));
      }
    })();
  }, [product, setIsFavorited]);

  const videoId = product?.video_products?.[0]?.video_id as string | undefined;
  const handleVideoEvent = async (eventName: 'VIDEO_START' | 'VIDEO_COMPLETE', metadata: Record<string, any> = {}) => {
    if (!product || !videoId) return;
    await trackEvent(eventName, { product_id: product.id, video_id: videoId, ...metadata }, { productId: product.id, categoryId: product.category_id || undefined });
  };
  const handleVideoProgress = async (seconds: number, duration: number) => {
    if (!product || !videoId || !Number.isFinite(seconds) || !Number.isFinite(duration) || duration <= 0) return;
    await trackEvent('VIDEO_PROGRESS', { product_id: product.id, video_id: videoId, seconds: Math.round(seconds), duration: Math.round(duration), progress_percent: Math.round((seconds / duration) * 100) }, { productId: product.id, categoryId: product.category_id || undefined });
    const { data: { user } } = await supabase.auth.getUser();
    if (user) await (supabase as any).from('video_watch_progress').upsert({ user_id: user.id, video_id: videoId, last_watched_seconds: Math.round(seconds), updated_at: new Date().toISOString() }, { onConflict: 'user_id,video_id' });
  };
  const handleBuyClick = async () => { if (!product) return; trackOutboundClick(product.id, 'external', product.affiliate_url || ''); if (product.discount && product.discount < 20) { setShowDownsell(true); trackEvent('DOWNSELL_TRIGGERED', { product_id: product.id, discount: product.discount }); } const { data: { user } } = await supabase.auth.getUser(); if (user) await trackInterestFn({ data: { userId: user.id, categoryId: product.category_id || 'general', action: 'click', metadata: { product_id: product.id } } }); if (product.affiliate_url) window.open(product.affiliate_url, '_blank', 'noopener,noreferrer'); };
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8 pb-16 sm:py-12">
    ...
    <div className="grid gap-10 lg:grid-cols-2 lg:gap-14"><div className="min-w-0 lg:sticky lg:top-24 lg:self-start"><ProductMediaShowcase title={product.title} images={product.images} discount={product.discount} video={videoUrl ? { id: video?.id, url: videoUrl, title: video?.title, subtitle: video?.subtitle, thumbnail: video?.thumbnail_url, resumeAt: videoResumeAt } : null} onVideoStart={() => void handleVideoEvent('VIDEO_START')} onVideoComplete={() => void handleVideoEvent('VIDEO_COMPLETE')} onVideoProgress={(seconds, duration) => void handleVideoProgress(seconds, duration)} /></div>
    <div className="min-w-0 space-y-8"><div className="space-y-5"><div className="flex flex-wrap items-center gap-3">{typeof product.rating === 'number' && product.review_count > 0 ? <><div className="flex gap-0.5" aria-label={`Nota ${product.rating} de 5`}>{Array.from({ length: 5 }, (_, index) => <Star key={index} className={`h-4 w-4 ${index < Math.round(product.rating) ? 'fill-amber-500 text-amber-500' : 'text-muted-foreground/20'}`} />)}</div><span className="font-black">{product.rating.toFixed(1)}</span><span className="text-xs text-muted-foreground">({Number(product.review_count).toLocaleString('pt-BR')} avaliações)</span></> : <span className="text-xs text-muted-foreground">Avaliação do marketplace ainda não disponível.</span>}</div><h1 className="flex flex-wrap items-center gap-3 text-4xl font-black uppercase italic tracking-tighter sm:text-5xl">{product.title}{product.is_best_offer && <Badge className="bg-amber-500 text-white"><Award className="mr-1 h-3 w-3" />Melhor Oferta</Badge>}</h1><div className="flex flex-wrap items-end gap-4"><span className="text-5xl font-black tracking-tight text-primary sm:text-6xl">R$ {(product.current_price ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>{product.previous_price && <span className="text-xl font-bold text-muted-foreground/40 line-through">R$ {product.previous_price.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>}</div></div>
