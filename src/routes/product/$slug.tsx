...
function ProductDetailPage() {
  ...
  const [targetPrice, setTargetPrice] = useState(''); const [alertOpen, setAlertOpen] = useState(false); const [showDownsell, setShowDownsell] = useState(false);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!user) return;
      const [{ data: favorite }, historyResult] = await Promise.all([
        supabase.from('favorites').select('id').eq('user_id', user.id).eq('product_id', product.id).maybeSingle(),
        supabase.from('browsing_history').select('id').eq('user_id', user.id).eq('product_id', product.id).maybeSingle(),
      ]);
      if (favorite) setIsFavorited(true);
      const { error } = await supabase.from('browsing_history').upsert({ user_id: user.id, product_id: product.id, viewed_at: new Date().toISOString(), device_type: /Mobi|Android/i.test(navigator.userAgent) ? 'mobile' : 'desktop' }, { onConflict: 'user_id,product_id' });
      if (error && !historyResult) console.debug('Browsing history unavailable until migration is applied.');
    })();
  }, [product, setIsFavorited]);

  const handleVideoEvent = async (action: string) => { if (!product) return; const { data: { user } } = await supabase.auth.getUser(); if (!user) return; try { await trackInterestFn({ data: { userId: user.id, categoryId: product.category_id || 'general', action, metadata: { product_id: product.id, video_id: product.video_products?.[0]?.video_id } } }); } catch (error) { console.error('Video tracking error:', error); } };
  const handleBuyClick = async () => { if (!product) return; trackOutboundClick(product.affiliate_url || '', product.id, 'external'); if (product.discount && product.discount < 20) { setShowDownsell(true); trackEvent('DOWNSELL_TRIGGERED', { product_id: product.id, discount: product.discount }); } const { data: { user } } = await supabase.auth.getUser(); if (user) await trackInterestFn({ data: { userId: user.id, categoryId: product.category_id || 'general', action: 'click', metadata: { product_id: product.id } } }); if (product.affiliate_url) window.open(product.affiliate_url, '_blank', 'noopener,noreferrer'); };
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8 pb-16 sm:py-12">
    ...
    <div className="grid gap-10 lg:grid-cols-2 lg:gap-14"><div className="min-w-0 lg:sticky lg:top-24 lg:self-start"><ProductMediaShowcase title={product.title} images={product.images} discount={product.discount} video={videoUrl ? { url: videoUrl, title: video?.title, subtitle: video?.subtitle, thumbnail: video?.thumbnail_url } : null} onVideoStart={() => void handleVideoEvent('video_start')} onVideoComplete={() => void handleVideoEvent('video_complete')} /></div>
