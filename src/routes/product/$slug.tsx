...
function ProductDetailPage() {
  ...
  const handleCreateAlert = async () => {
    if (!product) return;
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { toast.error('Faça login para criar um alerta de preço.'); return; }
    const value = Number.parseFloat(targetPrice);
    if (!Number.isFinite(value) || value <= 0) { toast.error('Informe um preço alvo válido.'); return; }
    try {
    ...
    } catch (error) { console.error('Price alert error:', error); toast.error('Não foi possível criar o alerta.'); }
  };
    ...
    <div className="container mx-auto max-w-7xl px-4 py-8 pb-16 sm:py-12">
      ...
      <div className="grid gap-10 lg:grid-cols-2 lg:gap-14">
        <div className="min-w-0 lg:sticky lg:top-24 lg:self-start">
          <ProductMediaShowcase title={product.title} images={product.images} discount={product.discount} video={videoUrl ? { url: videoUrl, title: video?.title, subtitle: video?.subtitle, thumbnail: video?.thumbnail_url } : null} onVideoStart={() => void handleVideoEvent('video_start')} onVideoComplete={() => void handleVideoEvent('video_complete')} />
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <Badge variant="outline" className="rounded-full border-glass-border bg-white/5 text-[9px]"><ShieldCheck className="mr-1 h-3 w-3" />Dados do produto</Badge>
          </div>
        </div>

        <div className="min-w-0 space-y-8">
          <div className="space-y-5">
            {typeof product.rating === 'number' && product.review_count > 0 ? <div className="flex flex-wrap items-center gap-3"><div className="flex gap-0.5" aria-label={`Nota ${product.rating} de 5`}>{Array.from({ length: 5 }, (_, index) => <Star key={index} className={`h-4 w-4 ${index < Math.round(product.rating) ? 'fill-amber-500 text-amber-500' : 'text-muted-foreground/20'}`} />)}</div><span className="font-black">{product.rating.toFixed(1)}</span><span className="text-xs text-muted-foreground">({Number(product.review_count).toLocaleString('pt-BR')} avaliações)</span></div> : <span className="text-xs text-muted-foreground">Avaliação do marketplace ainda não disponível.</span>}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Dialog open={alertOpen} onOpenChange={setAlertOpen}><DialogTrigger asChild><Button size="lg" variant="outline" className="h-14 rounded-2xl font-black uppercase tracking-wide"><BellRing className="mr-2 h-5 w-5" />Alerta de Preço</Button></DialogTrigger><DialogContent className="w-[92vw] max-w-md rounded-3xl"><DialogHeader><DialogTitle className="font-black uppercase italic">Vigilância de preço</DialogTitle><DialogDescription>Escolha o preço real que você deseja acompanhar.</DialogDescription></DialogHeader><div className="py-4"><Label htmlFor="target-price">Preço alvo</Label><div className="relative mt-2"><Target className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-primary" /><Input id="target-price" type="number" min="0.01" step="0.01" value={targetPrice} onChange={(event) => setTargetPrice(event.target.value)} className="h-12 pl-10" /></div></div><DialogFooter><Button onClick={() => void handleCreateAlert()} className="h-12 w-full rounded-xl font-black uppercase">Confirmar</Button></DialogFooter></DialogContent></Dialog>
