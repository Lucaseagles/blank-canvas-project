...
function ProductDetailPage() {
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8 pb-16 sm:py-12">
    ...
    <div className="grid gap-10 lg:grid-cols-2 lg:gap-14"><div className="min-w-0 lg:sticky lg:top-24 lg:self-start"><ProductMediaShowcase title={product.title} images={product.images} discount={product.discount} video={videoUrl ? { id: video?.id, url: videoUrl, title: video?.title, caption: video?.caption, subtitle: video?.subtitle_text || video?.subtitle, subtitleUrl: video?.subtitle_url, thumbnail: video?.thumbnail_url, resumeAt: videoResumeAt } : null} onVideoStart={() => void handleVideoEvent('VIDEO_START')} onVideoComplete={() => void handleVideoEvent('VIDEO_COMPLETE')} onVideoProgress={(seconds, duration) => void handleVideoProgress(seconds, duration)} /></div>
    ...
    {bridgeVideos.length > 0 && <section className="space-y-5 border-y border-glass-border py-7" aria-labelledby="product-bridge-videos"><div><h2 id="product-bridge-videos" className="text-xl font-black uppercase italic tracking-tight">Visto em vídeo</h2><p className="mt-1 text-xs text-muted-foreground">Conteúdo real curado nas plataformas de origem.</p></div><div className="grid gap-4 md:grid-cols-2">{bridgeVideos.map((bridgeVideo: any) => <VideoBridge key={bridgeVideo.id} video={bridgeVideo} />)}</div></section>}
