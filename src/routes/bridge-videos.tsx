  ...
  const { data: videos = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from('bridge_videos')
        .select('id,title,subtitle,thumbnail,platform,link,created_at,products!inner(id,slug,title,current_price,images,status)')
        .eq('is_active', true)
        .eq('products.status', 'active')
        .order('created_at', { ascending: false });
      if (error) throw error;
      return (data ?? []).map((row: any): BridgeVideo => ({
        id: row.id, title: row.title, subtitle: row.subtitle, thumbnail: row.thumbnail,
        platform: row.platform as BridgeVideoPlatform, link: row.link, product: row.products,
      }));
    },
  });
  ...
  return (
    <main className="container mx-auto max-w-7xl space-y-8 px-4 py-8 sm:py-12">
      <header className="max-w-2xl space-y-3">
        ...
        <p className="text-sm leading-relaxed text-muted-foreground sm:text-base">Assista ao conteúdo na plataforma original e encontre o produto correspondente aqui. Apenas vídeos reais cadastrados pelo administrador são exibidos.</p>
      </header>
      {isLoading ? (
