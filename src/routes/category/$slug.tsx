...
export const Route = createFileRoute("/category/$slug")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.slug.replace(/-/g, " ")} — Ofertas da Categoria` },
      { name: "description", content: "Ofertas verificadas por categoria." },
    ],
  }),
...
function CategoryPage() {
  ...
  const { data: subcategories = [] } = useQuery({
    queryKey: ["category-subs", category?.id],
    queryFn: () => getSubcategories(category!.id),
    enabled: !!category?.id,
  });
  const { data: breadcrumb = [] } = useQuery({
    queryKey: ["category-breadcrumb", category?.id],
    queryFn: () => getCategoryBreadcrumb(category!.id),
    enabled: !!category?.id,
  });
  ...
  return (
    <main className="container mx-auto max-w-7xl px-4 py-16 md:py-20 reveal-on-scroll">
      ...
      <header className="mb-10 mt-8 flex flex-col gap-6 rounded-[2rem] border border-border/60 bg-card/50 p-6 md:flex-row md:items-end md:justify-between md:p-8">
        <div>
          <Badge variant="outline" className="mb-4 border-primary/30 bg-primary/5 text-primary"><Globe className="mr-2 h-3.5 w-3.5" /> CATEGORIA</Badge>
          <h1 className="text-4xl font-black tracking-tighter md:text-7xl">{category.name}</h1>
        </div>
      ...
      <section className="mt-10">
        <div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-black">Produtos</h2><span className="text-sm text-muted-foreground">{filters.totalResults.toLocaleString("pt-BR")} resultados</span></div>
        {filters.loading ? <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="aspect-[4/5] rounded-[2rem]" />)}</div> : filters.results.length ? <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">{filters.results.map((p: any) => <ProductCard key={p.id} id={p.id} slug={p.slug || ""} categoryId={p.category_id} title={p.title} price={p.current_price ?? 0} previousPrice={p.previous_price} discount={p.discount} image={p.images?.[0] || ""} marketplace={p.marketplaces?.name || "External"} rating={p.rating} reviewCount={p.review_count} affiliateUrl={p.affiliate_url || null} isBestOffer={p.is_best_offer} offerScore={p.offer_score} />)}</div> : <div className="rounded-3xl border border-dashed border-border p-16 text-center text-muted-foreground">Nenhum produto encontrado com estes filtros.</div>}
      </section>
