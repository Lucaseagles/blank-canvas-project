...
import { Activity, BarChart3, CheckCircle2, ChevronRight, Layers3, Radio, RefreshCw, Target, Zap } from "lucide-react";
...
function ChannelStrategyMatrixPage() {
  ...
  const linkedRate = totals.strategies ? Math.round(((totals.collections + totals.campaigns) / (totals.strategies * 2)) * 100) : 0;
  ...
  return <div className="min-h-full bg-gradient-to-b from-muted/20 via-background to-background">
    <div className="mx-auto max-w-[1600px] space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Metric icon={Layers3} label="Coleções ligadas" value={totals.collections} detail={`${linkedRate}% de cobertura combinada`} />
      ...
      <Card className="overflow-hidden rounded-[2rem] border bg-card shadow-sm">
        <CardHeader className="border-b bg-muted/20 px-5 py-5 sm:px-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge variant="outline" className="rounded-full">{data?.channels.length ?? 0} canais</Badge>
          </div>
        </CardHeader>
