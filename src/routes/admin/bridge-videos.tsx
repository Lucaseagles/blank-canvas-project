...
const platforms = [
  { value: 'tiktok', label: 'TikTok' }, { value: 'shopee', label: 'Shopee' },
  { value: 'ml', label: 'Mercado Livre' }, { value: 'youtube', label: 'YouTube' },
] as const;
...
function AdminBridgeVideosPage() {
  ...
  const { data: videos = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      await assertOwner();
      const { data, error } = await (supabase as any).from('bridge_videos').select('id,title,subtitle,thumbnail,platform,link,is_active,created_at,product_id,products(id,title,current_price)').order('created_at', { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await (supabase as any).from('bridge_videos').insert({ title: form.title.trim(), subtitle: form.subtitle.trim() || null, thumbnail: form.thumbnail.trim() || null, platform: form.platform, link: form.link.trim(), product_id: form.productId, is_active: true });
      if (error) throw error;
    },
    onSuccess: () => { setForm({ title: '', subtitle: '', thumbnail: '', platform: 'tiktok', link: '', productId: '' }); queryClient.invalidateQueries({ queryKey: ['admin-bridge-videos'] }); toast.success('Vídeo ponte cadastrado.'); },
  ...
  const toggle = useMutation({
    mutationFn: async ({ id, active }: { id: string; active: boolean }) => {
      await assertOwner();
      const { error } = await (supabase as any).from('bridge_videos').update({ is_active: !active, updated_at: new Date().toISOString() }).eq('id', id);
      if (error) throw error;
    },
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => { await assertOwner(); const { error } = await (supabase as any).from('bridge_videos').delete().eq('id', id); if (error) throw error; },
  ...
  return (
    <main className="container mx-auto max-w-7xl space-y-8 px-4 py-8 sm:py-12">
      ...
      <Card className="border-glass-border bg-white/[0.03]"><CardHeader><CardTitle className="flex items-center gap-2"><Plus className="h-5 w-5 text-primary" />Cadastrar vídeo real</CardTitle></CardHeader><CardContent className="grid gap-5 md:grid-cols-2">
      ...
      <section className="grid gap-4">{isLoading ? <p className="text-sm text-muted-foreground">Carregando...</p> : videos.length === 0 ? <p className="rounded-2xl border border-glass-border p-6 text-sm text-muted-foreground">Nenhum vídeo ponte cadastrado ainda. Isso é esperado até o Owner cadastrar fontes reais.</p> : videos.map((video: any) => <Card key={video.id} className="border-glass-border bg-white/[0.03]"><CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center">{video.thumbnail ? <img src={video.thumbnail} alt="" loading="lazy" decoding="async" className="h-24 w-40 shrink-0 rounded-xl object-cover" /> : <div className="flex h-24 w-40 shrink-0 items-center justify-center rounded-xl bg-muted text-xs text-muted-foreground">Sem thumbnail real</div>}<div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2"><Badge>{video.platform}</Badge><Badge variant={video.is_active ? 'default' : 'outline'}>{video.is_active ? 'Ativo' : 'Inativo'}</Badge></div><h2 className="mt-2 font-black">{video.title}</h2><p className="text-sm text-muted-foreground">Produto: {video.products?.title || '—'}</p><a className="mt-1 block truncate text-xs text-primary hover:underline" href={video.link} target="_blank" rel="noopener noreferrer">{video.link}</a></div><div className="flex gap-2"><Button variant="outline" size="icon" onClick={() => toggle.mutate({ id: video.id, active: video.is_active })} aria-label="Ativar ou desativar"><Power className="h-4 w-4" /></Button><Button variant="outline" size="icon" onClick={() => { if (window.confirm('Remover este vídeo ponte?')) remove.mutate(video.id); }} aria-label="Remover"><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>)}</section>
    </main>
  );
}
