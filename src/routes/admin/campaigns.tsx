...
function AdminCampaignsPage() {
    ...
    saveMutation.mutate({
      data: {
        campaign: {
          ...
          status: (editingCampaign?.status || "scheduled") as any,
        },
        channels: channels as any[],
        productIds: [], // Future: Add product picker
      }
    });
  };
  ...
  return (
    <div className="p-4 lg:p-8 space-y-8 max-w-7xl mx-auto">
            ...
            <form onSubmit={handleCreate} className="space-y-6 pt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2 col-span-2">
                  ...
                  <Input name="name" defaultValue={editingCampaign?.name} required placeholder="Summer Launch 2026" className="rounded-xl border-glass-border bg-glass/50" />
                </div>
                <div className="space-y-2 col-span-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input name="starts_at" type="datetime-local" defaultValue={editingCampaign?.starts_at?.split('.')[0]} required className="rounded-xl border-glass-border bg-glass/50" />
                </div>
                <div className="space-y-2">
                  <Label className="uppercase font-black text-[10px] tracking-widest opacity-60">End Date</Label>
                  <Input name="ends_at" type="datetime-local" defaultValue={editingCampaign?.ends_at?.split('.')[0]} required className="rounded-xl border-glass-border bg-glass/50" />
                </div>
              </div>
                  ...
                  {['FEED_BANNER', 'PUSH', 'TELEGRAM', 'REFERRAL_BOOST'].map(channel => (
                    <div key={channel} className="flex items-center space-x-3 p-3 rounded-xl border border-glass-border bg-glass/30">
                      <Checkbox 
                        id={channel} 
                        name={channel} 
                        defaultChecked={editingCampaign?.campaign_channels?.some((c: any) => c.channel === channel)} 
                      />
              ...
              <div className="divide-y divide-glass-border">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-3">
                          <h3 className="font-black uppercase italic text-lg">{campaign.name}</h3>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl border-glass-border bg-glass backdrop-blur-xl">
                          ...
                          <DropdownMenuItem onClick={() => duplicateMutation.mutate({ data: { id: campaign.id } })} className="font-bold uppercase text-[10px] tracking-widest gap-2">
                            <Copy size={14} /> Duplicate
                          </DropdownMenuItem>
                          <DropdownMenuItem className="font-bold uppercase text-[10px] tracking-widest gap-2 text-rose-500">
                ...
                {(!campaigns || campaigns.length === 0) && (
                  <div className="p-12 text-center space-y-4">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest opacity-40">No active campaigns detected</p>
                  </div>
                )}
              </div>
            </CardContent>
                ...
                <div className="space-y-8">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <Tooltip 
                          ...
                          labelStyle={{ fontWeight: 'black', textTransform: 'uppercase', marginBottom: '4px' }}
                        />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                    {funnelData.map((item: any, idx: number) => {
                      const conversionRate = item.awareness_count > 0 
                        ? ((item.conversion_count / item.awareness_count) * 100).toFixed(1) 
                        : '0';
                      return (
                        <div key={item.channel} className="p-4 rounded-2xl border border-glass-border bg-glass/30 space-y-1">
                          <div className="flex items-center justify-between">
                            ...
                            {item.video_views > 0 && (
                              <span className="text-[8px] font-bold text-violet-400 uppercase tracking-tighter">
                                {item.video_views} Plays
                              </span>
                            )}
                          </div>
                          <div className="flex items-baseline justify-between">
                            <span className="text-xl font-black italic">{conversionRate}%</span>
                            <Badge variant="outline" className="text-[8px] font-bold border-emerald-500/20 text-emerald-500">
                              CONV
                            </Badge>
                          </div>
                        </div>
      ...
      <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-3xl p-6 flex gap-6 items-start shadow-2xl">
        <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 shrink-0">
          <ShieldCheck className="w-6 h-6" />
        </div>
        <div className="space-y-2">
          <h3 className="font-black text-xl tracking-tight uppercase italic">Compliance Safeguard Active</h3>
          <p className="text-muted-foreground text-[11px] leading-relaxed max-w-4xl uppercase font-bold tracking-tight">
            Neural filtering is enabled. All internal clicks from <span className="text-emerald-500 italic">owner</span> accounts are automatically excluded from funnel analytics to maintain conversion integrity. To test redirects without affecting metrics, ensure you are logged in as admin.
          </p>
        </div>
      </div>
    </div>
