...
function AdminOffersPage() {
  ...
  const getGroups = useServerFn(getOfferGroups); const getDetails = useServerFn(getOfferGroupDetails); const saveGroup = useServerFn(saveOfferGroup); const removeGroup = useServerFn(deleteOfferGroup);
  const [editing, setEditing] = useState<{ id: string; canonical_title: string } | null>(null); const [saving, setSaving] = useState(false);
  ...
  return <div className="container mx-auto py-12 px-4 max-w-7xl min-h-[80vh] space-y-10">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><Badge className="bg-primary/10 text-primary border-primary/20 font-bold px-3">Offer Intelligence</Badge><h1 className="mt-3 text-4xl sm:text-5xl font-black tracking-tighter uppercase italic">Direct Offers</h1><p className="mt-2 max-w-2xl text-muted-foreground">Gerencie grupos de ofertas, destaques e cupons reais.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => refetch()} disabled={isLoading}><RefreshCw className={isLoading ? "animate-spin" : ""} />Atualizar</Button><Button onClick={() => navigate({ to: "/admin/offers/new" })} className="h-11 rounded-2xl px-6 font-black uppercase italic gap-2"><Plus className="h-4 w-4" />New Offer</Button></div></div>
    <OfferFeaturedControl />
    <CouponAdmin />
