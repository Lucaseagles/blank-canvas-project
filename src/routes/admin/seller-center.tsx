...
import { BarChart3, ExternalLink, RefreshCw, Eye, MousePointerClick, Package, TrendingUp, Search, Filter, Link2 } from "lucide-react";
...
function SellerCenterPage() {
  ...
  return <main className="container mx-auto max-w-7xl space-y-7 px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div><Badge className="bg-primary/10 text-primary border-primary/20">Seller / Creator Center</Badge><h1 className="mt-2 text-4xl font-black italic uppercase tracking-tighter sm:text-6xl">Performance</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Visão operacional baseada em produtos reais e eventos reais dos últimos {periodDays} dias. A plataforma encaminha o usuário para o marketplace pelo link de afiliado.</p></div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center"><Select value={String(periodDays)} onValueChange={(value) => setPeriodDays(Number(value))}><SelectTrigger className="min-h-11 w-full rounded-xl sm:w-[140px]" aria-label="Período da performance"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="7">Últimos 7 dias</SelectItem><SelectItem value="30">Últimos 30 dias</SelectItem><SelectItem value="60">Últimos 60 dias</SelectItem><SelectItem value="90">Últimos 90 dias</SelectItem></SelectContent></Select><Button variant="outline" onClick={() => refetch()} disabled={isLoading} className="min-h-11 rounded-xl"><RefreshCw className={isLoading ? "mr-2 animate-spin" : "mr-2"} /> Atualizar</Button></div>
    </header>
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric icon={Package} label="Produtos" value={summary?.products ?? 0} /><Metric icon={TrendingUp} label="Publicados" value={summary?.published ?? 0} /><Metric icon={ExternalLink} label="Links afiliados" value={summary?.linkedProducts ?? 0} /><Metric icon={Eye} label="Visualizações" value={summary?.totalViews ?? 0} /><Metric icon={MousePointerClick} label="Cliques" value={summary?.totalClicks ?? 0} /><Metric icon={BarChart3} label="CTR" value={`${(summary?.ctr ?? 0).toFixed(2)}%`} /></section>
    <section className="flex flex-col gap-3 rounded-2xl border border-glass-border bg-glass-fallback p-3 shadow-sm sm:flex-row sm:items-center">
      <div className="relative flex-1">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar produto, slug ou categoria..." className="h-11 w-full rounded-xl border border-border/60 bg-background/60 pl-9 pr-4 text-sm outline-none transition focus:ring-2 focus:ring-primary/30" />
      </div>
      <Select value={statusFilter} onValueChange={setStatusFilter}>
        <SelectTrigger className="h-11 w-full rounded-xl sm:w-[190px]"><Filter className="mr-2 h-4 w-4" /><SelectValue placeholder="Status" /></SelectTrigger>
        <SelectContent>
