...
function OpportunitiesPage() {
  ...
  }).sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0)), [alerts, search, priority, type]);
  ...
  return <div className="min-h-full bg-gradient-to-b from-muted/20 via-background to-background">
    <div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between px-1"><div><p className="text-sm font-black">Fila inteligente</p><p className="text-xs text-muted-foreground">{filtered.length} de {alerts.length} oportunidades · maior prioridade primeiro</p></div><div className="hidden items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground sm:flex"><Clock3 className="h-3.5 w-3.5"/> revisão antes da ação</div></div>
...
function OpportunityCard({alert,pending,onDismiss,onAction}:{alert:OpportunityAlert;pending:boolean;onDismiss:()=>void;onAction:()=>void}) {
  ...
  const priorityLabel=priority>=70?"Alta":priority>=40?"Média":"Baixa";
  return <Card className={`group relative overflow-hidden rounded-[2rem] border bg-card shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg ${priority>=70 ? "ring-1 ring-primary/20" : ""}`}><CardContent className="p-0"><div className="absolute inset-y-0 left-0 w-1 bg-primary/20" /><div className="p-5 sm:p-6"><div className="flex items-start gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Lightbulb className="h-5 w-5"/></div><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2"><Badge className="rounded-full text-[9px] font-black uppercase">{typeLabels[alert.opportunity_type]??alert.opportunity_type}</Badge><Badge variant="outline" className="rounded-full text-[9px] font-black">Score {score.toFixed(0)}</Badge><Badge variant="outline" className={`rounded-full text-[9px] font-black ${priority>=70 ? "border-primary/40 text-primary" : ""}`}>Prioridade {priority} · {priorityLabel}</Badge></div><h3 className="mt-3 text-base font-black leading-5">{alert.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{alert.recommended_action}</p></div></div><div className="mt-4"><div className="mb-1 flex justify-between text-[9px] font-black uppercase tracking-widest text-muted-foreground"><span>Força do sinal</span><span>{score.toFixed(0)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:score+"%"}}/></div></div><div className="mt-5 rounded-2xl border bg-muted/20 p-3"><p className="mb-2 text-[9px] font-black uppercase tracking-[0.16em] text-muted-foreground">Por que apareceu</p><div className="flex flex-wrap gap-2">{(Array.isArray(alert.reasons)?alert.reasons:[]).filter(Boolean).map((reason:string)=><span key={reason} className="rounded-full border bg-muted/30 px-2.5 py-1.5 text-[10px] font-bold">{reason}</span>)}</div></div></div><div className="flex flex-wrap gap-2 border-t bg-muted/10 p-4"><Button size="sm" variant="outline" className="rounded-xl" onClick={onDismiss} disabled={pending}><XCircle className="mr-1.5 h-4 w-4"/>{pending ? "Salvando..." : "Dispensar"}</Button><Button size="sm" className="rounded-xl" onClick={onAction} disabled={pending}><CheckCircle2 className="mr-1.5 h-4 w-4"/>{pending ? "Processando..." : "Marcar tratada"}</Button></div></CardContent></Card>;
}
