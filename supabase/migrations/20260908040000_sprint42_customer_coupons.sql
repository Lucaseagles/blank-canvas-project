...
CREATE TABLE IF NOT EXISTS public.customer_coupons (
  ...
  CONSTRAINT customer_coupons_valid_window CHECK (valid_until IS NULL OR valid_until >= valid_from),
  CONSTRAINT customer_coupons_usage_limit CHECK (max_uses IS NULL OR used_count <= max_uses)
);
...
CREATE POLICY "customer_coupons_public_select" ON public.customer_coupons
  FOR SELECT TO anon, authenticated
  USING (is_active AND is_real AND valid_from <= NOW() AND (valid_until IS NULL OR valid_until >= NOW()) AND (max_uses IS NULL OR used_count < max_uses));

CREATE POLICY "customer_coupons_admin_select" ON public.customer_coupons
  FOR SELECT TO authenticated USING ((SELECT public.is_admin()));
CREATE POLICY "customer_coupons_owner_insert" ON public.customer_coupons
  FOR INSERT TO authenticated WITH CHECK ((SELECT public.is_admin()));
CREATE POLICY "customer_coupons_owner_update" ON public.customer_coupons
  FOR UPDATE TO authenticated USING ((SELECT public.is_admin())) WITH CHECK ((SELECT public.is_admin()));
CREATE POLICY "customer_coupons_owner_delete" ON public.customer_coupons
  FOR DELETE TO authenticated USING ((SELECT public.is_admin()));

CREATE OR REPLACE FUNCTION public.customer_coupons_set_updated_at()
...
CREATE OR REPLACE FUNCTION public.customer_coupons_enforce_real()
RETURNS TRIGGER LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN
  IF NEW.is_real IS DISTINCT FROM TRUE THEN RAISE EXCEPTION 'Somente cupons reais e confirmados podem ser publicados.'; END IF;
  NEW.code = UPPER(TRIM(NEW.code));
  IF NEW.code = '' THEN RAISE EXCEPTION 'O código do cupom é obrigatório.'; END IF;
  RETURN NEW;
END; $$;
