...
VALUES
  ('tiktok_shop', 'partner_id', 'Partner ID', 'text', TRUE),
  ('tiktok_shop', 'affiliate_id', 'Affiliate ID', 'text', TRUE),
  ('shopee', 'partner_id', 'Partner ID', 'text', TRUE),
  ('shopee', 'partner_key', 'Partner Key', 'password', TRUE),
  ('shopee', 'affiliate_id', 'Affiliate ID', 'text', TRUE),
  ('kwai', 'api_key', 'API Key', 'password', FALSE),
  ('mercadolivre', 'client_id', 'Client ID', 'text', TRUE),
  ('mercadolivre', 'client_secret', 'Client Secret', 'password', TRUE),
  ('mercadolivre', 'affiliate_id', 'Affiliate ID', 'text', TRUE),
  ('aliexpress', 'app_key', 'App Key', 'text', TRUE),
  ('aliexpress', 'app_secret', 'App Secret', 'password', TRUE),
  ('aliexpress', 'tracking_id', 'Tracking ID', 'text', TRUE),
  ('amazon', 'access_key', 'Access Key', 'text', TRUE),
  ('amazon', 'secret_key', 'Secret Key', 'password', TRUE),
  ('amazon', 'associate_id', 'Associate ID', 'text', TRUE),
  ('whatsapp', 'business_account_id', 'Business Account ID', 'text', TRUE),
  ('whatsapp', 'phone_number_id', 'Phone Number ID', 'text', TRUE),
  ('whatsapp', 'access_token', 'Access Token', 'password', TRUE),
  ('telegram', 'bot_token', 'Bot Token', 'password', TRUE),
  ('telegram', 'channel_id', 'Channel ID', 'text', FALSE)
ON CONFLICT (platform_id, field_key) DO UPDATE SET
  label = EXCLUDED.label,
  field_type = EXCLUDED.field_type,
  required = EXCLUDED.required;

-- Replace the original five-platform constraint with a registry foreign key.
ALTER TABLE public.integration_credentials
  DROP CONSTRAINT IF EXISTS integration_credentials_platform_id_check;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'integration_credentials_platform_id_fkey'
      AND conrelid = 'public.integration_credentials'::regclass
  ) THEN
    ALTER TABLE public.integration_credentials
      ADD CONSTRAINT integration_credentials_platform_id_fkey
      FOREIGN KEY (platform_id) REFERENCES public.integration_platforms(id);
  END IF;
END $$;
...
DECLARE
  ...
  description TEXT := 'Integration secret for ' || p_platform_id || '.' || p_field_key;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.integration_platforms WHERE id = p_platform_id AND active) THEN
    RAISE EXCEPTION 'Invalid or inactive integration platform';
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.integration_credential_fields WHERE platform_id = p_platform_id AND field_key = p_field_key) THEN
...
AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.integration_platforms WHERE id = p_platform_id) THEN
  ...
  ON CONFLICT (platform_id) DO UPDATE SET status = EXCLUDED.status, last_checked_at = EXCLUDED.last_checked_at, last_error = EXCLUDED.last_error, updated_at = NOW();
END;
$$;
